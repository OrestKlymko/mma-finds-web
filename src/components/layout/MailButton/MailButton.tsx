...
function MailButton() {
  ...
  const onSeeAllClick = () => {
    ...
    if (!user) return;
    if (user.type === 'manager') router.push('/manager/messages');
    if (user.type === 'provider') router.push('/provider/messages');
  };
  return (
    <div className="relative">
      ...
      {isDropdownOpen && (
        <DropdownWrapper
          className="flex flex-col items-center gap-4 mb:-right-32 right-0 w-96 shadow-2xl max-h-[600px] overflow-y-auto rounded-sm bg-white"
        ...
        >
          ...
          <div className="sticky bottom-0 bg-white p-4 flex justify-end w-full">
            <button className="text-green font-medium" onClick={onSeeAllClick}>
              See all
            </button>
          </div>
        </DropdownWrapper>
